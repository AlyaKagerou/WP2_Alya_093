...
class Penjualan extends CI_Controller{
    ...
    public function proses_tambah(){
        ...
        if($this->m_penjualan->tambah($data_penjualan) && $this->m_detail_penjualan->tambah($data_detail)){
            for($i=0; $i < $jumlah_barang_dibeli ; $i++){
                $this->m_barang->min_stok($data_detail_penjualan[$i]['jumlah_barang'], $data_detail_penjualan[$i]['nama_barang']) or die ('gagal min stok');
            } 
            $this->session->set_flashdata('succes', 'Invoice <strong>Penjualan</strong> Berhasil Dibuat');
                redirect('penjualan');
        } else {
            $this->session->set_flashdata('succes', 'Invoice <strong>Penjualan</strong> Berhasil Dibuat');
                redirect('penjualan');
    ...
    public function details($no_penjualan){
